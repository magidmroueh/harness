...
const SPARKLE_VARIANTS: Variants = {
  ...
  hover: { y: [0, -1, 0, 0], fill: "currentColor", transition: { duration: 1, bounce: 0.3 } },
};
...
// Sparkles uses two controllers so it can't use createAnimatedIcon
export const SparklesIcon = forwardRef<IconHandle, Props>(
  ({ onMouseEnter, onMouseLeave, className, size = 28, ...props }, ref) => {
    ...
    const handleMouseEnter = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
      if (isControlledRef.current) { onMouseEnter?.(e); } else { sparkleControls.start("hover"); starControls.start("blink", { delay: 1 }); }
    }, [onMouseEnter, sparkleControls, starControls]);
    const handleMouseLeave = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
      if (isControlledRef.current) { onMouseLeave?.(e); } else { sparkleControls.start("initial"); starControls.start("initial"); }
